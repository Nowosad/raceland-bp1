---
title: "An information theory-based approach for quantifing diversity and segregation"
...
Calculation of racial diversity (entropy) and racial segregation (mutual information) can be performed for the whole area of interests (i.e., metropolitan area) without introducing any arbitrary divisions or for many different spatial scales.
...
Our main goal is to measure racial diversity and racial segregation for different places in the city.
We can use the `get_raceland()` function for this purpose.

```{r, cache=TRUE}
results_metrics = get_raceland(race_raster,
                               n = 30,
                               window_size = 10, 
                               fun = "mean",
                               size = 20,
...
The `get_raceland()` function is a wrapper around several steps implemented in **raceland**, namely `create_realizations()`, `create_densities()`, `calculate_metrics()`, and `create_grid()`.
